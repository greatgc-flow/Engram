...
| `_sys/core/bootstrap.bat` | Reads the Python pin, bootstraps the first interpreter and pip, then enters the dispatcher ([`_sys/core/bootstrap.bat:21-38`](../../_sys/core/bootstrap.bat#L21-L38), [`_sys/core/bootstrap.bat:97-131`](../../_sys/core/bootstrap.bat#L97-L131), [`_sys/core/bootstrap.bat:160-163`](../../_sys/core/bootstrap.bat#L160-L163)). | A necessary bootstrap capability, not a reason for a permanent public `.bat` API. |
...
| Portable Python | Every `_sys/core/bootstrap.bat` invocation queries endoflife.date unless `--skip-update` is supplied ([`_sys/core/bootstrap.bat:62-95`](../../_sys/core/bootstrap.bat#L62-L95)). | A fresh install may rewrite the Python pin. An existing install is told to close tools, manually remove `_sys/env/python`, and rerun ([`_sys/core/bootstrap.bat:43-59`](../../_sys/core/bootstrap.bat#L43-L59), [`_sys/core/bootstrap.bat:71-88`](../../_sys/core/bootstrap.bat#L71-L88)). This is update behavior hidden inside install. |
...
There is no “install, then register, then launch” command choreography. The current quick start requires separate `_sys/core/bootstrap.bat` and `register.bat` steps ([`README.md:32-59`](../../README.md#L32-L59)); that sequence becomes a single first-run state machine. Menu integration stays optional because the environment works without it.
