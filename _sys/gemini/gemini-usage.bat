...
powershell -NoProfile -Command "$gd='%_GD:\=\\%'; $root='%_ROOT:\=\\%'; $today=Get-Date -Format 'yyyy-MM-dd'; $todayYMD=Get-Date -Format 'yyyyMMdd'; $gd=$gd.TrimEnd('\')+'\'; $root=$root.TrimEnd('\')+'\'; $tmpBase=$gd+'config\tmp'; $statusFile=$gd+'status.json'; $collabLog=$root+'_archive\collab-log\'+$today+'.md'; $outFile=$gd+'usage.json'; $sessions=@{}; $msgCount=0; $lastMsgTs=$null; if (Test-Path $tmpBase) { Get-ChildItem $tmpBase -Directory -ErrorAction SilentlyContinue | ForEach-Object { $lf=$_.FullName+'\logs.json'; if (Test-Path $lf) { try { $logs=Get-Content $lf -Raw | ConvertFrom-Json } catch { $logs=@() }; foreach ($e in $logs) { if ($e.timestamp -and $e.timestamp.StartsWith($today) -and $e.type -eq 'user') { $sessions[$e.sessionId]=1; $msgCount++; if (-not $lastMsgTs -or $e.timestamp -gt $lastMsgTs) { $lastMsgTs=$e.timestamp } } } } } }; $metrics=[pscustomobject]@{calls_today=0;last_axis=$null;last_call_ts=$null;consecutive_failures=0}; if (Test-Path $statusFile) { try { $s=Get-Content $statusFile -Raw | ConvertFrom-Json; if ($s.PSObject.Properties['gemini_metrics']) { $m=$s.gemini_metrics; if ($m.calls_today_date -eq $todayYMD) { $metrics=$m } } } catch {} }; $axisBreakdown=[ordered]@{A=0;B=0;C=0;'D+'=0;D=0;E=0;F=0;G=0;H=0}; if (Test-Path $collabLog) { Get-Content $collabLog | ForEach-Object { if ($_ -match '^## \[\d{2}:\d{2}:\d{2}\] (Axis-[A-H+]+)') { $ax=$Matches[1] -replace 'Axis-',''; if ($axisBreakdown.Contains($ax)) { $axisBreakdown[$ax]++ } } } }; $result=[ordered]@{date=$today;generated_at='%_DT%';gemini_cli_direct=[ordered]@{sessions_today=$sessions.Count;messages_today=$msgCount;last_message_ts=if($lastMsgTs){$lastMsgTs}else{$null}};axis_calls=[ordered]@{calls_today=[int]$metrics.calls_today;last_axis=$metrics.last_axis;last_call_ts=$metrics.last_call_ts;consecutive_failures=[int]$metrics.consecutive_failures;by_axis=$axisBreakdown};total_interactions_today=($sessions.Count+[int]$metrics.calls_today)}; [System.IO.File]::WriteAllText($outFile,($result | ConvertTo-Json -Depth 5),(New-Object System.Text.UTF8Encoding($false))); Write-Host 'OK: Usage written to' $outFile"
