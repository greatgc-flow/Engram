...
- A separate, directly-observed finding (not from any peer paper): `default:cx.effort`/`default:cx.deepthink` sessions had been continuously reused since 2026-07-16T17:04 across every unrelated debate that night; the single most recent `cx.deepthink` call's `cost-log.jsonl` entry showed **33,232,646 input tokens** for one ask. Confirmed by direct inspection of `_sys/codex/session_state.json` and `_sys/data/logs/cost-log.jsonl`, not a peer claim.
