...
## 14. Host Distribution, Packaging & User Lifecycle (added 2026-07-22)
...
*   **Three separate lifecycles, not one (MECE correction):** the clean partition is **Host Distribution Lifecycle** (Engram Core, `EngramHome`, the trust catalog, repair/update/uninstall of Engram itself), **Capability Lifecycle** (§13.5's bundles, plus external vendor CLI dependencies), and **Workspace Lifecycle** (create, open, migrate, relocate, retire, export, destroy). All three reuse §13.15's Governed Mutation Protocol once Policy exists, but remain separate domain state machines -- forcing them into one lifecycle would repeat the exact over-generalization mistake §13.15/§13.16 already rejected. Install and uninstall specifically must also work when the Core is ABSENT or BROKEN, so they require an installer/bootstrapper that lives outside `EngramApplication` itself (it cannot depend on the thing it's installing/repairing).
*   **`InstallationRecord` & host lifecycle:** an immutable record containing at minimum `installation_id`, `core_version`, `core_schema_version`, `runtime_version`, `trust_catalog_digest`, `builtin_bundle_set_digest`, `traceability_digest`, `active_release`, and `previous_release`. Host lifecycle: `absent -> staged -> verified -> installed -> active`, with `active -> updating -> active|rolled_back`, `active -> repairing -> active`, and `active -> uninstalling -> absent`. Core releases install side-by-side and activate through a single atomic pointer switch -- the SAME pattern already established for bundle upgrades (§13.5) and re-init (§13.9), applied one layer up. A release unit binds Core code, schemas, baseline Policy, the trust catalog, built-in bundle pins, and shipped `traceability.json` together as one atomic unit -- never independently versioned pieces that could drift out of sync with each other.
*   **Workspace compatibility across Core updates:** Workspace State and the `WorkspaceCatalog` gain explicit `workspace_schema_version` and `compatible_engine_api` fields. A Core update preflights the trust catalog, then migrates each workspace lazily -- on that workspace's FIRST OPEN after the update, not eagerly for every registered workspace at update time (Constraint 14 laziness, applied here too) -- via the same shadow-generation pattern already used for bundle-upgrade state migration (§13.5) and re-init (§13.9). A workspace found incompatible with the new Core fails closed without destroying its prior generation; it remains openable under a matching older Core release if one is still installed.
*   **Repair:** `engram repair` restores immutable release artifacts (Core code, schemas, trust catalog) from the installation's own verified source, while preserving Shared Config, Shared Data, credentials, and all workspaces by default -- repair fixes the Core installation, it does not touch user data unless a separate, explicit purge is also requested.
*   **Workspace retire/destroy/export are three different operations, not one "delete":** `workspace unregister`/`retire` removes a workspace from `WorkspaceCatalog` discovery while PRESERVING its `.ai/` on disk (the workspace still exists, it's just no longer actively tracked). `workspace destroy` is the actual data-deleting operation -- requires a dry-run preview, explicit confirmation, Policy authorization, and its deletion scope is hard-restricted to that one workspace's own `.ai/` (reusing §13.9's containment rule, never touching anything outside it). `workspace export`/`import` preserves identity (`workspace_id`) and provenance across the move, with explicit collision detection against the destination `WorkspaceCatalog` (reusing §13.8's duplicate-detection/rekey flow).
*   **Uninstall boundary (a real, not-implicit, default):** uninstalling Engram by default removes ONLY the Core installation itself -- program files, the embedded runtime, PATH aliases, installed Capability Bundles, and regenerable caches (`bundle-index.json` and similar per §13.10). Shared Data, credentials (via `CredentialResolver`, §13.5.5), and every workspace's `.ai/` are NEVER touched by a plain uninstall -- purging those requires a separate, explicit operation the user must request on purpose, never a side effect of removing the program.
*   **Multi-machine decision -- portable but single-writer for v1 (not concurrent multi-machine):** each machine installs Engram independently and maintains its own `WorkspaceCatalog`; the SAME `workspace_id` legitimately appearing in two different machines' `EngramHome`-scoped catalogs is NOT a duplicate (refining §13.8's duplicate-detection scope, which was already correctly limited to one `EngramHome`, with this explicit multi-machine framing). Portable workspace DEFINITIONS and provenance are kept separate from host-local paths, leases, running processes, and caches, so a workspace directory itself remains genuinely portable. Simultaneous mutation from unsynchronized copies on two machines is explicitly UNSUPPORTED in v1 and is detected (not silently allowed to corrupt) via a workspace generation/revision mismatch where possible. True concurrent multi-host use is deferred -- it needs distributed leases or a synchronization authority, which is a fundamentally different (and larger) problem than strengthening local CAS, and is out of scope here the same way third-party plugin distribution is deferred in §13.13. This is consistent with the original "install once, use from other apps" goal on ONE machine; it does not mean one installation serves unrelated machines concurrently.
*   **Offline boundary:** Engram's own control plane -- workspace operations, local Policy/trust-catalog validation, repair from local media, and any purely local-peer adapter -- must remain usable with no network access after initial installation. Initial installation and later updates support signed OFFLINE installation/update archives containing the complete release unit (matching §14's atomic release-unit bundling above). Whether a given VENDOR AI CLI can itself authenticate/operate offline is vendor-specific and explicitly marked **TEST NEEDED per vendor and account type** here -- e.g. Anthropic's own Claude Code setup documentation currently states an internet connection is required for Claude Code operation specifically [declared, unverified, per vendor docs] -- but that vendor-specific limitation must never be allowed to make ENGRAM's own workspace/diagnostic functions unnecessarily fail offline too.

### 14.1 Packaging & Distribution Decision

*   **Primary distribution -- a signed native installer with an embedded, private Python runtime:** for the Windows-first MVP (per §12's phasing), a direct signed per-user installer (`.exe`/`.msi`), later exposing the SAME signed payload through WinGet as an additional discovery channel, not a second build. Versioned, side-by-side application directories (mirroring the same atomic-release-pointer pattern used throughout §13/§14) with a small external bootstrapper handling install/update/rollback/repair/uninstall. The Python runtime and all dependencies are bundled privately -- Engram never depends on, or conflicts with, whatever Python the user's own machine happens to have. A stable `engram`/`engram.exe` shim on PATH always points at whichever release is currently active.
*   **Why not a single frozen executable (PyInstaller/Nuitka) as the PRIMARY distribution:** freezing the entire Core into one opaque binary would work for a small bootstrapper or rescue/repair tool, but conflicts with the rest of this architecture -- Capability Bundles (§13.5) need to be independently installable Python packages, `adapter-conformance/v1` (§13.12) needs ABI-keyed conformance receipts tied to a real, inspectable Python/platform combination, and §13.4's lazy handler imports need an actual import mechanism, not bytes frozen into a single-file archive. An embedded-but-real Python runtime under a native installer preserves all of this while still feeling like a normal installed application to the end user.
*   **Secondary channels, explicitly scoped:** **PyPI** is a developer/automation channel only (`pip install` into an isolated environment, e.g. via `pipx`/`uv tool` -- never a global `pip install` that could collide with the user's own Python packages), not the canonical end-user path. **Homebrew** (macOS) and **apt/dnf** (Linux) become real channels only once those platforms are actually supported per §12's phasing (a real Linux/macOS CI runner existing first), not before. **npm is explicitly NOT a canonical channel** -- Engram is not a Node application; wrapping it in an npm package would add a second runtime and a second, competing update authority for no architectural benefit (this is despite the current PortableDev setup already installing Claude Code's OWN CLI via npm-global -- that precedent applies to a vendor's Node-based tool, not to Engram's own Python-based Core). Whichever channel a given installation came through, that channel OWNS binary updates for that installation -- Engram must never race a package manager with its own competing self-updater.

### 14.2 Vendor AI CLI Boundary: Detect and Wrap, Never Bundle

*   **The rule:** Engram bundles its own adapters, manifests, schemas, and capability probes -- but NEVER the vendor AI CLIs themselves (Claude Code, Codex, Antigravity), their credentials, or their update mechanisms. This was already implied by §13.3's `python_bundle` adapter model (an adapter wraps an EXTERNAL vendor CLI, it doesn't reimplement or vendor one) and by the current PortableDev precedent (`orchestration.json`'s `hub_nodes` reference externally-installed binaries like `_sys\env\nodejs\npm-global\claude.cmd` and `agy.exe` that hub.py invokes but never installs) -- this section makes it an explicit, binding architectural rule rather than merely an implementation convention that could silently drift.
*   **The discovery/dependency model:** (1) the user installs each vendor CLI through that vendor's own official method -- Engram never intermediates this step; (2) `engram setup`/`engram doctor` DISCOVERS an installed vendor CLI through declarative locator rules (per-adapter, not hardcoded engine logic); (3) Engram records a dependency receipt keyed by the resolved binary's identity, observed version/digest, platform, the adapter bundle's own digest, and a capability-probe result; (4) a missing or incompatible dependency produces an explicit diagnostic plus a link to the vendor's own official installation instructions -- never a silent failure or an Engram-authored workaround install; (5) any binary/version drift on the vendor's side invalidates the old dependency receipt and triggers a reprobe/re-conformance-check before further use (this is the concrete mechanism that closes the "vendor CLI changes independently" gap the lifecycle audit found -- Engram notices drift rather than silently trusting stale assumptions); (6) authentication/login/logout for a vendor CLI remain entirely vendor-owned -- Engram's `CredentialResolver` (§13.5.5) stores only authorized credential REFERENCES where an adapter genuinely needs one, never re-implements a vendor's own auth flow.
*   **Formalized in the adapter manifest:** an adapter's `bundle.json` (§13.5) declares an explicit external-dependency contract -- supported vendor-CLI version range, discovery strategy, capability-probe definition, a reference link to the vendor's own official help/install docs, and an explicit statement that install/update/auth for that dependency are vendor-owned, not Engram-owned. This rule applies equally to future `generic_cli`/`DeclarativeCliAdapter` peers (§13.3) once that adapter kind is actually implemented -- a declarative adapter still wraps an externally-installed CLI, it does not change who owns installing it.
*   **Evidence basis (declared, unverified -- vendor docs, not independently verified this session):** OpenAI documents Codex CLI installation through official scripts, npm, Homebrew, and release binaries; Anthropic documents native installers, WinGet, Homebrew, and npm for Claude Code, with installation-method-dependent update behavior; Google maintains its own separate Antigravity installation/authentication workflow. All three vendors already have independent, self-owned installation and update authorities -- Engram taking ownership of any of them would couple Engram's own release cycle to three external publishers it doesn't control, and create real ambiguity over whose signature/auth/rollback/support applies to a given binary.
