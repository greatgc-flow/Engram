...
def ensure_peer_cli(peer: str, orch: dict | None = None, sys_dir: Path | None = None, force: bool = False) -> dict:
    ...
    if mechanism in ("exe_tool", "zip_tool", "sfx_exe"):
        ...
        if not force and _already_current(dest_dir, manifest_path, tool_cfg, declared_version, bin_name):
            return {"status": "already_current", "detail": "Version matches manifest"}
    ...
    if mechanism == "npm_peer":
        ...
        # Invoke npm via "node.exe npm-cli.js" directly rather than npm.cmd.
        # npm.cmd (auto-generated by npm itself, not ours to edit) does its own
        # prefix-detection via a `for /f ('CALL "%NODE_EXE%" ...')` line -- the
        # same cmd.exe metacharacter-reparse hazard as any for/f command
        # substitution: an "&" anywhere in the portable root's path (a real
        # folder name hit 2026-09-04) breaks that line before npm ever runs.
        # Calling npm-cli.js straight through node.exe is a plain argv list
        # with no batch re-parsing, so it's immune regardless of path content.
        # Tradeoff: this always uses the bundled npm-cli.js, never a
        # self-updated global npm at a configured prefix -- fine here since
        # this npm install is entirely provisioner-managed, never user-updated.
        npm_cli_js = env_dir / "nodejs" / "node_modules" / "npm" / "bin" / "npm-cli.js"
        npm_argv = [str(node_exe), str(npm_cli_js)]
        try:
            res = subprocess.run(
        ...
        try:
            subprocess.run(
                [*npm_argv, "install", "-g", f"{pkg}@{declared_version}"],
        ...
        if not ok:
            if is_update and old_version and old_version != declared_version:
                try:
                    subprocess.run(
                        [*npm_argv, "install", "-g", f"{pkg}@{old_version}"],
                        env=npm_env, check=True,
                    )
