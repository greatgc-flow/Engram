---
name: context-health
description: "Check Claude context window health. Runs Axis-H (check-health.bat) to measure JSONL size vs GREEN/YELLOW/RED thresholds. Reports status and recommends /compact or new session. Use when: context feels slow, before a heavy multi-file task, or after a long session."
---
...
1. Run Axis-H:
   Bash: `_sys\checks\check-health.bat`
...
3. Report to user:
   ...
   **RED** (> 1.2 MB):
   ...
   Option 2: New session — reads _archive/session-handoff.json to resume
   Run check-health.bat --force first to generate the handoff file."

4. If --force or status is RED:
   Bash: `_sys\checks\check-health.bat --force`
   Session handoff written to _archive/session-handoff.json. Report path to user.
