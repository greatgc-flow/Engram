...
> **Reconciled with §13.9 (fixed 2026-07-23, cx.deepthink finding, MEDIUM):** this section's example previously contradicted §13.9 in two ways -- it located the template at a fixed Core-shipped path rather than resolving it through `ResourceRegistry`, and it included a `.gitignore` managed-block destination OUTSIDE `<workspace>/.ai/`, directly violating §13.9's containment rule (every Base Template destination must resolve beneath the workspace's own `.ai/`; an outside destination is a hard error). Both cannot be true at once. Resolution: Core ships this JSON as an IMMUTABLE SEED template (`core_templates/init_workspace.json`, below, is the seed's shape); host installation registers/copies that seed into Shared Config as a versioned template resource during `engram workspace init` (§14); RUNTIME initialization resolves only the Shared Config `resource_id` via `ResourceRegistry`, per §13.9 -- never the Core-shipped path directly. The `.gitignore` managed-block entry is removed from the Base Template itself, since it targets a destination outside the allowed containment root; if that capability is still wanted, it becomes a SEPARATE, explicitly optional operation (e.g. `engram workspace integrate gitignore`) with its own dry-run, explicit confirmation, Policy authorization, path/symlink containment relative to the specifically selected project file, and its own governed-mutation/audit receipt per §13.15 -- not a hidden exception folded into the Base Template.
...
*   **§14 artifact Store/Plane classification (fixes a real gap, cx.deepthink finding, MEDIUM, 2026-07-23):** §13.1 requires every Engram-owned artifact to carry both a Store and a Plane classification; §14 introduced several without ever assigning them.
    ...
    | `HostUpdateAttempt` / `WorkspaceCompatibilityPlan` | Shared Data | Evidence |
    | Transition and recovery receipts (`InitialInstallationReceipt`, break-glass receipts, etc.) | Shared Data | Evidence |
    | `InstallationMutationLease` / admission fence | Shared Data | Mechanism |
    | Release manifest and the bootstrapper itself | Core Engine | Mechanism |
    | `ExternalDependencyReceipt` / `PeerReadinessReceipt` (§14.2) | Shared Data | Evidence (the authoritative readiness BINDING itself is Binding plane, distinct from the receipt evidence) |
    | External bootstrap journal, before `EngramHome` exists | *(declared exception, below)* | *(declared exception, below)* |

    The external bootstrap journal is a declared exception, not an undeclared sixth persistent store -- no Engram store exists yet before the very first installation, so it is explicitly temporary OS-installer transaction state, imported into proper Evidence (Shared Data) once `EngramHome` exists, then retired. These artifacts also need `resource_id`/`schema_id` values and §13.7 traceability entries once their concrete schemas are written (Phase 3) -- without that, §14 would silently escape the classification and traceability rules the rest of the document depends on.

*   **Three separate lifecycles, not one (MECE correction):** the clean partition is **Host Distribution Lifecycle** (Engram Core, `EngramHome`, the trust catalog, repair/update/uninstall of Engram itself), **Capability Lifecycle** (§13.5's bundles, plus external vendor CLI dependencies), and **Workspace Lifecycle** (create, open, migrate, relocate, retire, export, destroy). Forcing them into one lifecycle would repeat the exact over-generalization mistake §13.15/§13.16 already rejected. **Every Engram-owned authoritative transition in all three lifecycles uses §13.15 once Policy exists (corrected 2026-07-23, cx.deepthink finding, HIGH -- see §13.15's own scope-broadening fix); external facts/effects and the explicitly named pre-runtime bootstrap paths do not.** A normal Core update, for instance, begins as a real `MutationRequest`:
    ```text
    MutationRequest {
      subject_kind: "host_installation", subject_id: engram_home_id,
      subject_scope: "host", operation: "core_update",
      expected_revision: current_record.record_id,
      idempotency_key, correlation_id, actor, policy_revision,
      payload: { candidate_release, workspace_compatibility_plan_digest,
                 expected_host_lifecycle_epoch, installation_channel,
                 update_authority }
    }
    ```
    then: (1) Policy authorizes it against the resolved immutable Policy revision; (2) domain validation checks update authority, candidate authenticity, migration coverage, and rollback availability; (3) the AUTHORIZED request is durably handed to the external bootstrapper before quiescence even begins -- the bootstrapper does not invent or widen the operation, it accepts only the already-authorized subject/candidate/expected-revision/actor/idempotency-key; (4) `HostUpdateAttempt` plus lease/fence acquisition plus final activation form a SAGA of governed record transitions sharing one `correlation_id` (this makes `HostUpdateAttempt` resumable recovery state with durable governed phase changes, not merely §13.15's lightweight `AuditedOperation`); (5) final activation commits the new `InstallationRecord`, its fence/lock delta, post-commit effect intents, and the outbox receipt through §13.15's `MutationPlan`, exactly as §13.5's bundle upgrades and emergency quarantine already do; (6) the resulting `TransitionReceipt` follows `committed_enforcement_pending -> completed` as post-commit checks and admission-reopening finish (§13.15's own status vocabulary, reused here); (7) a Policy revision change discovered before final activation requires reauthorization or abort per the operation's declared policy-staleness rule, never silent proceed on a stale authorization. Normal repair, rollback, uninstall, purge, and retained-home mutation follow the identical mapping. **Workspace destroy (§14) is likewise expressed through §13.15, not a look-alike parallel mechanism:** `active|retired -> destroy_pending` is a governed `MutationRequest`; the filesystem rename to a quarantine location is an external saga step outside per-record atomicity (matching §13.15's own per-record-not-distributed atomicity boundary); `destroy_pending -> destroyed` is a second governed mutation; permanent deletion is a post-commit effect intent driven by the durable outbox; all steps share one `operation_id`/`correlation_id`.
