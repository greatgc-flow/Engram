...
*   **The migration snapshot must be taken AFTER draining, not concurrently with it (closes a remaining lost-write race, cx.effort finding, 2026-07-22):** blocking new admissions while in-flight processes merely "finish on their own in the background" is not sufficient by itself -- a write from an old-generation process that lands AFTER the state snapshot for migration was already taken, but BEFORE that process exits, would silently vanish (new invocations only ever see the new generation; the late write was never migrated and was never in the new generation either). The maintenance lease must actively DRAIN: wait for every in-flight writer against the bundle to actually exit before the state snapshot for migration is taken, not merely stop admitting new ones while migration proceeds in parallel. A writer that will not exit within a bounded timeout is surfaced as an explicit stuck-writer condition requiring an administrator force-stop decision (logged to the Evidence plane, §13.6) -- the upgrade must not silently time out into either an inconsistent migration or an indefinite hang.
...
*   **Fully-qualified key for cross-workspace traceability (closes a residual gap, cx.effort finding, 2026-07-22):** a bare `resource_id` unique only within its own scope (above) is NOT globally identifiable, but §13.7's `traceability.json` graph is a single global structure that may need to reference resources from multiple workspaces. Fix: within `traceability.json` (and anywhere else a resource must be referenced unambiguously outside its own scope), a `scope: workspace` entry is addressed by a fully-qualified key, `workspace://<workspace-identity>/resource/<resource_id>`, not the bare `resource_id` alone; `core`/`shared` entries, having only one registry to begin with, may keep using the bare `resource_id`. `owner` and `retention` (§13.5.5's other fields) are likewise scope-relative for workspace entries: a workspace-scoped resource is owned and retained together with that workspace and does not outlive it unless something explicitly exports it into a `core`/`shared` entry.
...
`bundle-index.json` and `traceability.json` are generated, non-authoritative artifacts. They do not fit the ordinary contracts of Core Engine, Capability Bundle, Workspace State, Shared Config, or append-only Shared Data. They are classified together here because both are DERIVED (neither is a primary source of truth), but they are NOT regenerated the same way or at the same time -- treating them as one interchangeable "cache" was itself a category error (fixed 2026-07-22, ag.deepthink finding), so this section now covers two distinct sub-kinds under one shared classification.

*   **Classification:** Derived artifacts are an explicitly documented pattern (non-authoritative, atomically replaced rather than appended, deletion never destroys unique information since everything in them is reconstructible from some other source of truth) applied across TWO different store homes, not a sixth store (corrected 2026-07-22, cx.effort finding: an earlier version of this section put both sub-kinds in Shared Data, which contradicted sub-kind B's own "ships with each Core release" description below -- Store answers "where," and the two sub-kinds do not share a "where").
*   **Sub-kind A -- `bundle-index.json` (runtime-regenerable cache, home: Shared Data):** rebuilt at runtime, on the installed machine. Rebuilding from installed manifests ALONE is not sufficient (cx.effort finding, 2026-07-22): a rebuild must cross-check every installed manifest's digest against the Core-release trust catalog (§13.11) and reject/exclude any bundle whose digest is not authorized there, THEN atomically write the index -- rebuilding purely from local manifests, with no reference to the shipped trust catalog, would let a rebuild silently "verify" a tampered local manifest, defeating the exact trust purpose the index exists for. Its regeneration is DELIBERATELY NOT automatic/lazy-on-miss despite being cheap to rebuild -- a missing/stale index fails closed (§13.4) rather than silently regenerating, specifically because this index is trust-relevant (it backs §13.4's TOCTOU integrity checks) and silently rebuilding trust state on every miss could mask real tampering or corruption instead of surfacing it. Regeneration requires the explicit `engram plugin verify --all` trigger.
