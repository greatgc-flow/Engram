<div align="center">
  ...
  [![Tests: 1179 green](https://img.shields.io/badge/tests-1179%20green-brightgreen.svg)](_sys/tests/unit)
</div>
...
- **Validation:** tiered TDD suite under [`_sys/tests/unit`](_sys/tests/unit) — `l1_core`, `l2_policy`, `l3_mocked` — **1179 passing** of 1185 collected, plus consistency checks (`check_cli_reality`, `check_cli_canary`, `check_contracts`, `check_sandbox_behavior`, `check_operational_guard_matrix`, `check_peer_characteristics`, `check_peer_capability_canary`, `check_capability`). The 6 non-passing tests are known pre-existing issues tracked in `_sys/ai/backlog.json` (T72, T73) — confirmed unrelated to any change landing alongside this count. Test count last verified 2026-07-19.
