...
*   **Classification:** Derived artifacts are an explicitly documented sub-area of Shared Data, not a sixth store. Common contract for both sub-kinds: non-authoritative and never the only copy of a fact, atomically replaced rather than appended, and deletion never destroys unique information (everything in them is reconstructible from some other source of truth).
*   **Sub-kind A -- `bundle-index.json` (runtime-regenerable cache):** rebuilt at runtime, on the installed machine, from installed bundle manifests alone -- everything it needs is already present locally. Its regeneration is DELIBERATELY NOT automatic/lazy-on-miss despite being cheap to rebuild -- a missing/stale index fails closed (§13.4) rather than silently regenerating, specifically because this index is trust-relevant (it backs §13.4's TOCTOU integrity checks) and silently rebuilding trust state on every miss could mask real tampering or corruption instead of surfacing it. Regeneration requires the explicit `engram plugin verify --all` trigger.
*   **Sub-kind B -- `traceability.json` (CI/build-time artifact, not runtime-regenerable):** requires `source_symbol`/`test_ref` references into the full Core Engine source tree and test suite (§13.7) -- inputs that do not exist on an ordinary installed/runtime machine, which typically has installed bundles and manifests but not the Core Engine's own source checkout. It is therefore generated once, during CI/the build-and-release pipeline that DOES have the full source tree, and ships as a read-only artifact WITH each Core release rather than being rebuilt on demand at runtime the way `bundle-index.json` is. `engram plugin verify --all` (sub-kind A's regeneration path) does not, and cannot, rebuild it.
*   **Store contract:** Shared Data documentation must distinguish its normal append-only Evidence records from this derived sub-area, and distinguish the two derived sub-kinds' regeneration stories (runtime-triggered vs. CI-time-shipped) from each other.
