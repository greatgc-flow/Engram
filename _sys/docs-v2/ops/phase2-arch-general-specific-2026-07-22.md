...
*   **Lifecycle:** A bundle proceeds through `available -> verified -> installed-disabled -> enabled`, with `enabled <-> disabled`.
    *   **Incompatible branch:** `verified` may enter `incompatible` (a bundle that fails schema/compatibility checks before ever being installed).
