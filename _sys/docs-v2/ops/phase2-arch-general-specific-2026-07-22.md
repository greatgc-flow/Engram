...
*   **Stuck-writer timeout, fully specified (closes an underspecified transition, cx.deepthink finding, 2026-07-22):** a writer that does not exit within a bounded drain timeout transitions the upgrade attempt to an explicit `blocked_stuck_writer` state -- no snapshot is taken and no migration proceeds from this state. The default action on timeout is ABORT the upgrade attempt (the old bundle/generation remains active and untouched); this is not a silent hang and not a silent proceed. Only a Policy-authorized administrator may request a force-stop of the stuck writer (never automatic). If force-stop is authorized, the engine terminates the writer's process tree and then explicitly CONFIRMS every writer lease against the bundle is actually released before retrying the snapshot -- if confirmation fails (a lease is still held, e.g. termination was incomplete), the attempt aborts again rather than proceeding on an unconfirmed assumption.
...
*   **Two distinct roots, resolved separately (corrected 2026-07-22, cx.deepthink finding):** the original single precedence chain conflated two roots that must never be conflated -- an untrusted workspace's `.ai` directory must never be able to redirect where Core, Shared Config, Shared Data, or Capability Bundle actually live. There are two independent resolution chains:
    ...
    *   **`WorkspaceCatalog` and init ordering (closes a discovery gap, cx.deepthink finding, 2026-07-22):** duplicate-ID detection and any cross-workspace aggregation (§13.5.5) require actually knowing which workspaces exist -- an arbitrary copied-then-abandoned workspace directory can't be discovered by magic. `EngramHome` maintains a `WorkspaceCatalog` (a Shared Data record: `workspace_id` -> last-known `WorkspaceRoot` path), updated whenever a workspace is initialized or opened; cross-workspace queries iterate this catalog rather than scanning the filesystem, and a `WorkspaceRoot` not reachable at its last-known path is reported stale rather than silently skipped. `engram workspace init`'s precise order: resolve `WorkspaceRoot` -> validate the Base Template (§13.9) -> generate `workspace_id` in a staging area (not yet committed) -> create any template-specified registry/Evidence entries using that `workspace_id` (still staged) -> atomically commit the whole Workspace State directory into place (§13.9's containment/staging rule) -> register the new `workspace_id`/path pair in the `WorkspaceCatalog`. Re-running init against an already-initialized workspace preserves its existing `workspace_id` rather than generating a new one.
