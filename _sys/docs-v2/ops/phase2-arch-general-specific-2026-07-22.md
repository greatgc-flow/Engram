...
*   **Every abort path has the same mandatory cleanup, not just the "old generation remains active" claim (closes a gap, cx.deepthink finding, 2026-07-22):** ANY abort of an upgrade attempt -- whether from a pre-install gate failure, a stuck-writer timeout, or a failed force-stop confirmation -- must, before the attempt is considered finished: release the maintenance lease (restoring normal admission of new work against the UNCHANGED old bundle/generation -- an abort that leaves the lease held would silently and permanently prevent the old, still-active bundle from accepting any new work, which is worse than the original upgrade failure); discard or explicitly mark the candidate `state_generation` inactive (it is never left in an ambiguous half-adopted state); and durably record the aborted attempt (Evidence plane, §13.6). "The old generation stays active" is only true in practice if the lease release actually happens -- it is not automatic just because the transaction never committed.
