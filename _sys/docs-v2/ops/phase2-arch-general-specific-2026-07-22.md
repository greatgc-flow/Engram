...
*   **Bootstrap authority -- the first installation is NOT a Governed Mutation (fixes a real gap, cx.deepthink finding, HIGH, 2026-07-22):** §13.15's Policy authorization presupposes `RuntimeContext` already exists, which presupposes Policy is already loaded, which presupposes `EngramHome` already exists -- so the FULL first transition (`absent -> staged -> verified -> installed -> active`, below) must stay entirely OUTSIDE the Governed Mutation Protocol; it cannot switch into §13.15 partway through. This first install is a `BootstrapInstallTransaction`, trusted axiomatically relative to Engram's own future Policy -- but it is still AUTHENTICATED: the external bootstrapper validates the release manifest and publisher signature against an installer-embedded or OS-level trust anchor, never against the trust catalog that this very transaction is about to install (§13.11's digest-only bundle verification is explicitly NOT sufficient authenticity for the Core installer itself -- that mechanism was designed to verify bundles trusted BY an already-installed Core, not to bootstrap the Core's own trust). The transaction atomically installs the release unit, creates baseline Policy and the first `InstallationRecord` generation (below), and activates the first release; it writes an `InitialInstallationReceipt` to an installer-owned journal, which the first successful `EngramApplication` start then imports into Evidence (§13.6) as the first entry a Governed Mutation history has to work with. **A second bootstrap-adjacent boundary:** repair or uninstall when Core or Policy itself is CORRUPT cannot use §13.15 either, even though Policy files nominally still exist on disk -- define a `break-glass` route, distinct from normal Policy-authorized repair/uninstall below: the external bootstrapper authenticates the OS user/administrator plus signed recovery media, records an external recovery receipt, and never claims Policy authorization occurred. "Repair from the installation's own verified source" is unsafe precisely when that source or its trust catalog is what's damaged -- genuine recovery uses an independently signed archive, a package-manager cache, or the original installer, validated by the bootstrapper's OWN external trust anchor, never by asking the possibly-compromised installation to vouch for itself.
*   **`InstallationRecord` as immutable generations, not a record that's both immutable and mutated in place (fixes a real contradiction, cx.deepthink finding, MEDIUM, 2026-07-22):** an "immutable record" cannot itself be the thing whose `active_release`/`previous_release` fields get updated -- each host mutation instead writes a NEW immutable generation (carrying `installation_id`, `installation_revision`, `host_lifecycle_epoch`, `active_release`, `previous_release`, `shared_state_generation`, `core_schema_version`, `runtime_version`, `trust_catalog_digest`, `builtin_bundle_set_digest`, `traceability_digest`, plus the channel-guarantee fields defined in §14.1) and CAS-switches a `current_record` pointer to it -- the same generation+pointer-swap pattern used everywhere else in §13/§14. The old generation remains the actual rollback/audit source. Host lifecycle STATE is kept separate from update-ATTEMPT outcomes (mirroring §13.5's bundle-state vs. upgrade-attempt-state split): `active -> updating -> active | aborted` -- a failed update is an ABORTED ATTEMPT, and the host remains `active` throughout, pointing at whichever release generation is actually current (not a persistent "rolled_back" installation state). `active -> repairing -> active` and `active -> uninstalling -> ...` (see the uninstall-boundary fix below) follow the same state-vs-attempt separation, with interrupted attempts requiring the same explicit abort cleanup and durable attempt-record as §13.5's bundle upgrades.
...
*   **Workspace compatibility across Core updates -- bounded, not open-ended (fixes an overclaim, cx.deepthink finding, HIGH, 2026-07-22):** Workspace State and the `WorkspaceCatalog` carry explicit `workspace_schema_version` and `compatible_engine_api` fields; a Core update migrates each workspace LAZILY, on that workspace's first open after the update (Constraint 14), via the same shadow-generation pattern as bundle-upgrade migration. The earlier claim that an incompatible workspace "remains openable under a matching older Core release if one is still installed" was broader than the record actually supports -- with only one `previous_release` retained, a workspace last touched two Core versions back (e.g. still at schema v1 after Core went v1->v2->v3, with only v2 retained) would have no matching release to fall back to, even though the CLAIM implied it always would. Fix, an MVP rule that stays bounded rather than promising unlimited history: before activating a new Core release, the update inspects every ACTIVE catalog entry's recorded workspace schema; the candidate new release must declare a valid direct migration path for every reachable schema present (even though actual execution of that migration stays lazy per above); before REPLACING `previous_release` with the outgoing release, the update verifies every still-unmigrated workspace CAN be migrated by the new release -- if any cannot, activation is BLOCKED and the affected workspace must be migrated, exported, or retired first; a workspace that's unreachable or stale at update time requires an explicit compatibility waiver rather than being silently assumed safe. `previous_release` therefore supports only IMMEDIATE installation-wide rollback together with its paired `shared_state_generation` -- restated precisely: "a failed lazy migration may trigger immediate rollback to the retained previous release and its matching shared-state generation," not an open-ended promise of arbitrary historical per-workspace Core execution. (If genuinely unbounded historical fallback is ever required, `previous_release` would need to become `retained_releases[]`, each paired with its own compatible state generation -- substantially more complex, and explicitly deferred rather than half-implemented here.)
